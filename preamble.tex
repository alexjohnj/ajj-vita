\setlength\parindent{0pt}

\newenvironment{itemize*}%
{\begin{itemize}%
    \setlength{\itemsep}{0pt}}%
  {\end{itemize}}

\usepackage{url}
\usepackage{fancyhdr}
\usepackage{lastpage}
\usepackage{enumitem}
\setlist{nolistsep,leftmargin=0.15in}
\usepackage{booktabs}
\usepackage[log-declarations=false]{xparse}
\usepackage[adobe-garamond]{mathdesign}
\usepackage[no-math]{fontspec}
\usepackage{fontawesome}
\usepackage{color}
\usepackage{luatexbase} % temporary  until microtype is updated
\usepackage{microtype}
\usepackage[a4paper,top=25mm,right=25.4mm,left=50.8mm]{geometry}
\usepackage[strict]{changepage}
\usepackage[
ocgcolorlinks,
urlcolor={[rgb]{0,0,0.54}},
unicode,
plainpages=false,
pdfpagelabels,
pdftitle={\mytitle},
pdfauthor={\myauthor},
pdfkeywords={\mykeywords}
]{hyperref}

\setmainfont{Source Sans Pro}
\setsansfont{Source Sans Pro}

\newcommand{\ML}{\textsc{Matlab}}

\definecolor{header}{RGB}{173,16,47}
\definecolor{keyword}{RGB}{173,16,47}

\newcommand{\mhead}[1]{\leavevmode\marginpar{\flushright\scshape\sffamily \textcolor{header}{#1}}}
\newcommand{\rdate}[1]{{\addfontfeature{Numbers=OldStyle} \hfill #1}}
\renewcommand{\date}[1]{{\addfontfeature{Numbers=OldStyle} #1}}
\renewcommand{\labelitemi}{-}
\DeclareTextFontCommand{\skill}{\bf\color{keyword}}

\makeatletter % fix for \hrulefill w/ mathdesign package
\def\hrulefill{\leavevmode\leaders \hrule height \rulethickness \hfill\kern\z@}
\makeatletter

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "vita"
%%% End:
